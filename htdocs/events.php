...
// This is used for most urls


$client_id = $Sanitizer->get_int('client_id', 0);
$events_last = $Sanitizer->get_string('events_last');
$express_filter = $Sanitizer->get_string('express_filter');
$filter_id = $Sanitizer->get_string('filter_id');
$init = $Sanitizer->get_int('init', 0);
$journal_button = $Sanitizer->get_string('journal_button', FALSE);
$journal_id = $Sanitizer->get_int('journal_id', -1, FALSE);
$map_id = $Sanitizer->get_int('map_id', 0);
$order_type = $Sanitizer->get_string('order_type', 'desc');
$refresh = $Sanitizer->get_int('refresh',FALSE);
$show_all = 0;
$span = $Sanitizer->get_int('span', 15);
$sound = $Sanitizer->get_int('sound');
$view_type = $Sanitizer->get_string('view_type', 'html');
$view_mode = $Sanitizer->get_string('view_mode', 'normal');
...
if ($refresh!='0') { //if refresh is not set to disable (0)
  if (!$refresh) $refresh=profile('EVENTS_REFRESH'); //get refresh from profile
        if (!$refresh) $refresh=$Config->get('events_refresh'); //if it was not set use the default
    } else 
  $refresh='';
    
$Events = new JffnmsEvents();
if ($journal_button !== FALSE)
  $journal_id = add_journal($Events, $journal_id);
...
"    function go_filter(select) {
  ...
  echo tag('tbody', 'body');
    ...
    $events_raw = $Events->get_all(NULL,$map_id,$have_filter,$filter,$init,$span_extended,'desc',$view_all,$show_all, 0, $client_id);
  ...
  for ($i = 0; ($i < $cant_events) || (($view_all==1) && (is_array($events[$event_pos]))) ; $i++)
    ...
    show_event ($event, $map_id, $filter_id);
  ...
  switch ($view_type) {
    ...
    case 'html':
      // Sound Execution
      if ($sound==1)
        if (isset($events_actual) && ($events_actual > $events_last) && ($events_last))
          echo play_sound($events_sound);
      if (isset($events_actual))
        $url = get_events_url(array('events_latest' => $events_actual));
      else
        $url = get_events_url(FALSE);
      echo javascript_refresh("if (self.no_refresh!=1) location.href=\'$url\';",$refresh);
      adm_footer();
      break;
    case 'rss':
    case 'rdf': 
      echo tag_close('channel'). tag_close('rss');
      break;
    }
...
{
  global $Config, $Sanitizer, $url_delete_tags;
  global $order_type, $view_type, $view_all;
  ...
  case 'html':
    $output = 
        tr_open ('','header').
        td ('Date'. linktext(image($order_image),
          get_events_url(array('order_type' => $order_change))).
        ...
        table ('','buttons').
        tr_open ().
        td (linktext(image('a-top.png'), get_events_url(array('init'=>0)))).
        td (($init > 0)
        ?linktext(image('a-left.png'), get_events_url(array('init'=>($init-$span),'refresh=0'))):'').
        td (linktext(image('a-right.png'), get_events_url(array('init'=>($init+$span),'refresh'=>0)))).
        td ((empty($refresh)
          ?linktext(image('refresh.png'), get_events_url(array('refresh'=>1)))
          :image('refresh2.png', NULL, NULL,'Refresh','','',
        'if (self.no_refresh!=1) {
            self.no_refresh = 1; 
            this.src = \'images/refresh.png\'; 
        } else 
            document.location = \''.htmlspecialchars(get_events_url(FALSE)).'\';
        '))).
      
        td (linktext(image('plus.png'), get_events_url(array('span'=>$span*2)), array('span'))).
        td (linktext(image('minus.png'), get_events_url(array('span'=>$span/2)), array('span'))).
        td ((($view_all==0) 
        ?linktext(image('all.png'), get_events_url(array('view_all'=>1,'refresh'=>0)))
          :linktext('- '.image('all.png'), get_events_url(array('view_all'=>0))))).
        td ((($sound==1)
          ?linktext(image('sound.png'), get_events_url(array('sound'=>0)))
          :linktext(image('nosound.png'), get_events_url(array('sound'=>1))))).

        td ('Event', 'detail').
        td (
          linktext(image('csv.png'), get_events_url(array('view_type'=>'csv', 'view_all'=>1)), '_new').
          linktext(image('filter4.png'), 
            "javascript: popUp('".$Config->get('jffnms_rel_path')."/admin/event_filter.php?order_type=".
    ...
    $output .=
      ...
      html('title', 'JFFNMS', '', '', '', false, true).
            html('link', current_host().$Config->get('jffnms_rel_path'), '', '', '', false, true).
...
function events_verify_summary ($event)
{
  ...
  if (array_key_exists('index', $summary) && $summary['index']==$index)
  {
    ...
    if (array_key_exists('event', $summary) && is_array($summary['event']))
    {
...
function get_events_url($new_tags)
{
  global $Sanitizer;
  $delete_tags = array('ticket_button','comment','subject', 'checkedid','filter_list', 'journal_id', 'journal_button');
  return $Sanitizer->get_url('', 'all', $new_tags, $delete_tags);
}

function add_journal (&$Events, $journal_id)
{
  global $Sanitizer;

  $checked_ids = $Sanitizer->get_string('checkedid');
  $subject = $Sanitizer->get_string('subject', '');
  $comment = $Sanitizer->get_string('comment', '');
  ...
  if (!empty($comment))
  {
    ...
    $timed_comment = "|".$now.",".$_SESSION['auth_user_id']."|\n".$comment."\n";
  
    if (($journal_id==1) && (!empty($subject))) //new journal
    {
      $journal_data = array('date_start'=>$now, 'subject'=>$subject);
      $journal_id = $Journals->add();
    } elseif ($journal_id > 2) //update journal
    {
      unset($journal_data);
      $journal_data = array('date_stop' => $now);
    }
      
    if (is_array($journal_data) && ($journal_id > 1)) 
      $Journals->update($journal_id,$journal_data,$timed_comment);
  }
  if (($journal_id > -1) && is_array($event_ids) && (count($event_ids) > 0))
  {
    foreach ($event_ids as $event_id) 
      $Events->set_ack($event_id, $journal_id);
  }
  return $journal_id;
} //add_journal()
...
function show_journal($journal_id)
{
  ...
  if ($journal_id > 1) 
    $journal_active = current($Journals->get_all($journal_id,1));
  else
    $journal_active = array('subject'=>'','comment'=>'');
  
  return 
    table('journal').
    table_row('Journal','title', 3, '', false).

    tr_open('selector').
    td (select_journal('journal_id',$journal_id,1, array(1=>'New Journal'))).
