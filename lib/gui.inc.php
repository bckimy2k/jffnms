  ...
  if (is_array($value))
   ...
   /* 
    function clean_url($extra_vars = array(),$no_vars = array()) {
      var_dump($extra_vars, $no_vars);
      die("Cannot clean url, use sanitizer");
  if ($GLOBALS["_GET"]) 
...
function http_authenticate()
{
  ...
  include($Config->get('jffnms_real_path').'/htdocs/logout.php');
  exit;
}
...
-->\n";
  ...
  echo 
    ...
    :"").
      ...
      tag("link","","","rel='shortcut icon' type='image/x-icon' href='$jffnms_rel_path/images/jffnms.ico'", false).
      tag("link","","","rel='icon' type='image/x-icon' href='$jffnms_rel_path/images/jffnms.ico'", false).
      tag("link","","","rel='stylesheet' href='$jffnms_rel_path/default.css'", false).
      (!empty($GLOBALS["jffnms_custom_css"])?tag("link","","","rel='stylesheet' href='".$GLOBALS["jffnms_custom_css"]."'", false):"").
      tag("link","","","rel='alternate' type='application/rss+xml' title='".$Config->get('jffnms_site')." Events Feed' href='$jffnms_rel_path/events.php?view_type=rss'", false).
      tag_close("head"). 
      ((!$just_head)
    ?tag ("body","","",
  ...
  echo
    ...
    {
      $news = http_post_message($news_url,array());
      if (!empty($news))
        return @explode("\n",$news);
      else 
        return false;
...
{
  global $Config;
  $file = $Config->get('jffnms_rel_path').'/sounds/'.$file;
...
function show_event (&$event, $map_id,$filter_id)
{
  global $Sanitizer, $Config;
  global $REQUEST_URI, $express_filter, $view_type, $journal_id;

  $jffnms_rel_path = $Config->get('jffnms_rel_path');
  $image_ack = (($event['ack']>0)?'ok.png':'alert2.gif');

  $filter_url = $Sanitizer->get_url('','all',array('filter_id'=>$filter_id));
  ...
  case 'html':
    ...
    $ack_url = $Sanitizer->get_url('','all',
      array('journal_id'=>($event['ack']==0?1:0), 'journal_button' => 'Ack',
      'checkedid[]' => $summary_ids));

    $output = tr_open("event$id", (($event["ack"]!=0)?"ack":"event"), $bgcolor, "", "color: ".$fgcolor).
    ...
    td( checkbox_value('checkedid[]', $summary_ids, 
        ...
        linktext($ack_image, $ack_url), 'ack_check').
...
function performance_graph ($id, $graph_filename, $graph_function, $sizex, $sizey, $title, $graph_time_start, $graph_time_stop, $other_data = "")
{
  jffnms_load_api('rrdtool');

  $retval = rrd_grapher($id, $graph_function, $sizex,$sizey, $title,$graph_time_start,$graph_time_stop, $other_data);
  if ($retval == FALSE)
    return FALSE;
  list ($value, $graph_data, $other_data) = $retval;
  ...
  return $value;
    ...
    function news_parse ()
    {
      global $jffnms_site, $info;
      $i_info = (is_array($info) && array_key_exists('interfaces', $info)?$info['interfaces']['data']:'');
      return array('site'=>$jffnms_site, 'version'=>JFFNMS_VERSION,
        'h'=>$info['hosts']['data'],
        'i'=>$i_info, 
...
function alarms_get_status ($alarms)
{
  ...
  if (is_array($alarms))
    ...
    {
      if ($key=='total')
        $alarms_parsed[$key]='T='.$alarm['qty'];
      else
        $alarms_parsed[$key]=strtoupper($key[0]).$alarm['qty'];
...
function ImageStringCenter($im,$color,$y,$text,$big = 0)
{
  ...
  return "http".(isset($_SERVER["HTTPS"])?"s":"")."://".$_SERVER["HTTP_HOST"];
    ...
    function display_error($errtitle,$errmsg="") {
        return('
<style>
div.errmsg {
