...
class JffnmsInterfaces extends basic
{

  public function get_by_field_value($host_id, $interface_type, $field_name, $field_value)
  {
    $query="
      SELECT
        i.id as interface_id,i.interface as interface
      FROM
        interfaces_values iv, interface_types_fields itf, interfaces i
      WHERE
        iv.field=itf.id AND iv.interface=i.id AND
        itf.name='$field_name' AND itf.itype=$interface_type AND
        i.host=$host_id AND value='$field_value'";
    ...
    if (is_array($filters))
      foreach ($filters as $filter_type=>$filter_value)
        if ($filter_value)
          ...
          case 'id':
          case 'type':
            $where[] = array("interfaces.$filter_type", '=', $filter_value);
            break;
          case 'interface':
            $where[] = array("interfaces.interface", '=', "'$filter_value'");
            break;
          case 'zone':
            $where[] = array('hosts.zone','=',$filter_value);
            break;
          case 'map':
            if ($filter_value > 1)
            {
          ...
          case 'in_maps':
            if ($filter_value==1)
              $where[]=array("interfaces.show_rootmap","!=",0);
            break;
          case 'custom':
            if (is_array($filter_value))
              $where=array_merge($where,$filter_value);
            break;
          case 'with_graph':
            $where[] = array('graph_types.type','=','interfaces.type');
            $from[] = 'graph_types';
            ...
            break;
          case 'graph_fields':
            $where[]=array('interface_types.graph_default','=','graph_types.id');
  ...
  public function alarms($ids = NULL, $filters = NULL, $alarm_filters = NULL, $start=FALSE, $stop=FALSE)
  {
    ...
    list ($where_special,$aux_from) = $this->filter($filters);
    $aux_from = join(', ',$aux_from);

    if (array_key_exists('only_active', $filters) && $filters['only_active']==1)
    ...
    if (count($ids) > 0)
    {
      ...
      if (isset($where_ids_aux))
        $where .= ' and ((1=2)'.$where_ids_aux.')';
    }

    $query = "$fields \n$from \n$joins \n$where \n$group \n$order";
    //debug($query);

    $result = db_query($query) or die ('Query Failed - Interface->alarms - '.db_error());
    return $result;
  } //alarms()

  public function status($ids = NULL, $filters = NULL)
  {
    $result = $this->alarms ($ids, $filters);

    if (!isset($filters['alarms_summary']))
      $filters['alarms_summary'] = 1;  //Summary is Default
    if (!isset($filters['show_disabled']))
      $filters['show_disabled'] = 1;  //By Default Show Disabled Interfaces
    ...
    while ($res = db_fetch_array($result))
    {
      //debug ($res);
      if (!$summary)
        if (array_key_exists($res['interface_id'], $all_status))
          $status = $all_status[$res['interface_id']];
        else
          $status = array();

      //set total to 0
      if (!array_key_exists('total', $status) || !array_key_exists('qty', $status['total']))
        $status['total']['qty'] = 0;
      ...
      {
        if (!array_key_exists($res['alarm'], $status))
          $status[$res['alarm']] = array('qty'=>0);
      ...
      if (!$summary)
        $all_status[$res['interface_id']] = $status;
    }//while res
    if (!$summary)
      $status = $all_status;
    //debug ($status);
  ...
  public function is_up($interface_id)
  {
    ...
    $result = db_query($query) or die ("Query Failed - Interfaces=>is_up - ".db_error());
  ...
  public function get_all($ids = NULL, $filters = array(), $count = 0)
  {
    $index = "interfaces.id";

    list ($where_special,$aux_from,$aux_fields, $aux_index, $aux_order) = $this->filter($filters);
    ...
    if ($count)
      ...
      if (count($result) > 0)
      {
        ...
        if (array_key_exists('with_field_type', $filters) && $filters['with_field_type']==1)
  ...
  public function interface_count ($ids = NULL, $filters = NULL, $count = 0)
  {
  ...
  function update($id,$interface_data)
  {
    ...
    // Dont change mod date if we are updating last poll date
    if (key($interface_data)!='last_poll_date')
      $interface_data["modification_date"]=time();
    ...
    $data = $this->values($id,array('exclude_types'=>20));
    ...
    if (is_array($fields) && (count($fields) > 0))
    {
      ...
      foreach ($fields as $field_name => $field_data)
      {
        ...
        {
          if ($new_value == $field_data['default_value'])
          {
            $this->delete_value ($id,$field_data["id"]);
          } else {
            if ($old_value == $field_data["default_value"])
            {
              //debug ("Adding $field_name to $new_value in interface $id field ".$field_data["id"]);
              $this->insert_value ($id,$field_data["id"],$new_value);
           } else {
              //debug ("Updating $field_name to $new_value in interface $id field ".$field_data["id"]);
              $this->update_value ($id,$field_data["id"],$new_value);
           }
          }
        }
    ...
    if (key($interface_data)!='last_poll_date')   //if we are not just updating the last poll date
      $this->adjust_rrd_max($id);     // adjust the RRDs MAX values
    return $result;
  } //update()
  ...
  {
    global $Config;
    $Maps_interfaces = new JffnmsMaps_interfaces();
    //RRD Deletion
    ...
    if ($type_info['rrd_mode']==1)
    {
      ...
      $dss = $this->parse_rrd_structure($type_info["rrd_structure_def"]);
    ...
    $result["delete-values-$id"] = $this->delete_value($id);
    $result["delete-db-$id"] = db_delete("interfaces",$id);
    $result["delete-from-maps-$id"] = $Maps_interfaces->del_from_all($id);
    return $result;
  } //del()

  function add($host_data=NULL)
  {
    $host_data['creation_date'] = time();
    return db_insert('interfaces',$host_data);
  }


  // Private functions - rrdtool
  public function get_type_info ($interface_id)
  {
    $query = "
        SELECT
    ...
    if (db_num_rows($result) == 1)
    {
      ...
      $normal_data = $this->values($interface_id,array('exclude_types'=>20));
      $normal_data = current($normal_data["values"]);

      $rrd_data = $this->values($interface_id,array('ftype'=>20));
      ...
      if (is_array($rrd_data))
      {
        ...
        foreach ($normal_data as $key=>$value) //replace variables for max in rrd
          $register['rrd_structure_def'] = str_replace("<$key>",$this->calculate_rrd_max($value),$register["rrd_structure_def"]);
      }
      return $register;
    }
    return FALSE;
  ...
  private function calculate_rrd_max ($max)
  {
    $max = (($max * 1.5)/8)+1; //1.5 times the bigger bw in bytes, plus 1
    return round($max);
  }

  private function adjust_rrd_max($interface_id)
  {
    global $Config;
    jffnms_load_api('rrdtool');

    $info = $this->get_type_info($interface_id);
    if ($info == FALSE)
      return;

    $rrd_structure_def = explode(' ',trim($info['rrd_structure_def']));
    $filename = $Config->get('rrd_real_path')."/interface-$interface_id.rrd";

    if ((is_array($rrd_structure_def)) && (count($rrd_structure_def) > 0)) //if it has a RRD Structure and more than 1 DS
      foreach ($rrd_structure_def as $dsn=>$ds)
      {
        $aux = explode(':',$ds);
        $filename_ds = str_replace(".rrd","-$dsn.rrd",$filename);

        if ($info['rrd_mode']==1)
          $result[] = rrdtool_tune($filename,'-a '.$aux[1].':'.$aux[5]);
        if ($info['rrd_mode']==2)
          $result[] = rrdtool_tune($filename_ds,'-a data:'.$aux[5]);
      }
  } //interface_adjust_rrd_max()

  public function parse_rrd_structure($structure)
  {
  ...
  public function values($ids = NULL,$filters = array())
  {
    $fields = array();
    $values = array();
    ...
    $query= "
        SELECT
          ...
          f.id as field_id, f.description, f.name,  f.showable,
          f.overwritable, f.default_value, f.ftype,
          v.value,
          it.update_handler, it.autodiscovery_parameters,
          i.type,  i.id
        FROM
    ...
    while ($r = db_fetch_array($result))
    {
      $fields [$r['type']][$r['name']] = array(
        ...
        'autodiscovery_parameters'=>$r['autodiscovery_parameters'],
      );

      if ($r['value'] == NULL)
        $values[$r['id']][$r['name']] = $fields[$r['type']][$r['name']]['default_value'];
      else
  ...
  private function update_value ($interface, $field, $value)
  {
    ...
    return db_query($query) or die ("Query Error - ".db_error());
  }

  private function insert_value ($interface, $field, $value)
  {
    if (!is_numeric($interface) || !is_numeric($field))
      return FALSE;
  ...
  private function delete_value ($interface, $field = NULL)
  {
  ...
  function get_availability($ids, $date_from_unix, $date_to_unix, $alarm_types, $detail = 0)
  {
    $info = array('interfaces'=>array(),
      'summary'=>array('interfaces'=>0)
    );
  ...
  list( $info['summary']['unavail_seconds'], $info['summary']['outages']) = $this->calculate_unavail_time($unav);
