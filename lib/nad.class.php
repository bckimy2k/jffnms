...
class JffnmsNad
{
  ...
  function get_data ($params = array())
  {
    if (array_key_exists('zone', $params))
      $zone_where = ' AND zones.id='.$params['zone'];
    else
      $zone_where = '';
    ...
    while ($rnad = db_fetch_array($result_nad))
    {
      if (!isset($hosts[$rnad['host_id']]))
        $hosts[$rnad["host_id"]] = array(
          'ips'=>array(), 'snmp_name'=>$rnad['snmp_name'], 
      ...
      return array($hosts, $ips, $networks, $net_ids);
    }
    return FALSE;
  }
      
  function graph_host_id ($id, &$hosts)
  {
    return (($hosts[$id]['forwarding']==1)?'H':'I').$id;
  }
      
  function graph_host ($id, &$hosts, &$ips) {
  
    $idg = $this->graph_host_id($id, $hosts);
   ...
   function graph_data ($graph)
    {
      global $Config;
      $graph_output = join("\n",array_unique($graph));
      $neato_path = $Config->get('neato_executable');
      $path_real = $Config->get('engine_temp_path');
    ...
    function graph_init ($bgcolor = "white")
    {
    ...
    ");
      ...
      function graph_by_host ($hosts, $ips, $networks, $host_id="", $show_all = 0) {
    $hosts_ids = array();
    ...
    foreach ($host_cnx as $host_id=>$host_data) {
        ...
        list ($host_idg, $graph[]) = $this->graph_host($host_id, $hosts, $ips);
        
        if (isset($host_data["networks"]))
        foreach ($host_data["networks"] as $net=>$aux) {
      
      list($net_idg, $graph[]) = $this->graph_net($networks[$net]["id"],$net);
      ...
      foreach ($host_data["hosts"] as $host2_id=>$net)
          $graph[] = "$host_idg -- ".$this->graph_host_id($host2_id, $hosts);
          }
    
    return $graph;
      }
  
  function graph_by_network (&$hosts, &$ips, &$networks, $network = '')
  {
    $hosts_ids = array();
      ...
      if ($aux1['no_more'] != 1)
        foreach ($aux1['hosts'] as $aux2=>$host_id) 
          foreach ($hosts[$host_id]['ips'] as $net2=>$aux3)
            if ($net1!=$net2)
              if (($this->get_bitmask($net2) < 30) && ($net2!=$network)) 
    ...
    foreach ($net_cnx as $net1=>$aux)
      if ($this->get_bitmask($net1)==30)
      {
        if (count($aux['hosts'])==2)
        {
          $h1 = $aux["hosts"][0];
          $h2 = $aux["hosts"][1];
          $graph[] = $this->graph_host_id($h1, $hosts).' -- '.$this->graph_host_id($h2, $hosts).' [headlabel=\'$net1\']';
  
          $hosts_ids[$h1]=1;
          $hosts_ids[$h2]=1;
        }
      } else {
        list ($net1_idg, $graph[]) = $this->graph_net($networks[$net1]['id'], $net1);
        foreach ($aux["hosts"] as $host_id)
        {
          $graph[] = "$net1_idg -- ".$this->graph_host_id ($host_id, $hosts);
          $hosts_ids[$host_id]=1;
        }
      }
    foreach ($hosts_ids as $host_id=>$aux)
      list(,$graph[]) = $this->graph_host($host_id, $hosts, $ips);
  
    return $graph;
  } //naf_graph_by_network()
  
  function get_bitmask ($prefix)
  {
  ...
  function graph($params = array())
  {
    ...
    $graph_init = $this->graph_init($params["bgcolor"]);
  
    if (isset($params['network']))
        $graph_core = $this->graph_by_network ($hosts, $ips, $networks, $params['network']);
    else 
        $graph_core = $this->graph_by_host ($hosts, $ips, $networks, $params['host_id'], $params['view_all']);
  
          $graph_finish = $this->graph_finish();
  
    if (count($graph_core)!=0) {
  
        $graph = array_merge($graph_init, $graph_core, $graph_finish);
        $output = $this->graph_data($graph);
