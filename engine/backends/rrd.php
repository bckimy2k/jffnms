...
function backend_rrd($options,$result)
{
  ...
  $filename = $Config->get('rrd_real_path').'/interface-'.$options['interface_id'].'.rrd';
  $type_info = $Interfaces->get_type_info($options['interface_id']);
  ...
  $dss = $Interfaces->parse_rrd_structure ($type_info['rrd_structure_def']);
  $num_ds = count($dss);
