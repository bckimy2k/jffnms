...
if (count($masters)<1)
  $masters=array($my_sat_id); //Only One
...
{ 
  if ($my_sat_id!=$master) //I'm not a master
  {
    ...
    if (is_array($paths))
    {
      ...
      foreach ($paths as $path)
        $paths_description[] = join(array_reverse($path),'->');
  ...
  if ($cant > 0) //ok we have some destinations
  {
    ...
    if (($cant_masters > 1) || ($my_sat_id != $master))
      logger(' :  H '.str_pad($host_id,3,' ',STR_PAD_LEFT).' : Using direct connexion to Satellite '.$keys[$pos].": ".$peer_to_master[$master][$keys[$pos]][url]."\n");
...
if (is_array($peer_to_master)) 
  while ($poller_data = poller_plan_next($poller_plan_result))
  {
    ...
    if ((file_exists($poller_filename)==TRUE) && (file_exists($backend_filename)==TRUE))
    {
      ...
      foreach ($peer_to_master as $master_id=>$peer)
      {
        ...
        if (isset($result['times']))
          $times[$master_id]=$result['times']; //copy the result profiling data
    ...
    if (array_key_exists('poller_parameters', $poller_data))
      $poller_param_description = $poller_data['poller_parameters'];
    else
    ...
    //debug output  
    logger(
      ' :  H '.str_pad($host_id,3,' ',STR_PAD_LEFT).
  ...
  foreach ($working_paths as $master_id=>$aux) foreach ($aux as $aux1)
    foreach ($aux1 as $peer_sat_id=>$peer_sat)
      if (isset($peer_sat['session'])) //if we have established a session with this peer
      {
        ...
        if ($peer_to_master[$master_id][$peer_sat_id]["session"])
          $use='Used';
        else
          $use='Tested'; 
        if (!is_array($result['session_destroy']))
          $result['session_destroy'] = array($result);
...
{
  $longopts = array ('help', 'version', 'host:', 'interface:',
    'poller:', 'type:');
  if ( ($opts = getopt('?vh:i:p:t:', $longopts)) === FALSE)
    return;
  if (sizeof($opts) == 0)
  {
    switch($_SERVER['argc'])
    {
    case 6:
      $itype = $_SERVER['argv'][5];
      if (!is_numeric($itype))
        die("Interface type \"$itype\" must be a number");
    case 5:
      $poller_pos = $_SERVER['argv'][4];
      if (!is_numeric($poller_pos))
        die("Poller position \"$poller_pos\" must be a number");
    case 4:
    case 3:
      $interface_id = $_SERVER['argv'][2];
      if (!is_numeric($interface_id))
        die("Interface ID \"$interface_id\" must be a number");
    case 2:
      $host_id = $_SERVER['argv'][1];
      if (!is_numeric($host_id))
        die("Host ID \"$host_id\" must be a number");
    }//switch
    return;
  } 
...
function print_help($errmsg='')
{
  ...
  print
"
Usage: php -q poller.php [options]
 JFFNMS Consolidator

  -i,--interface ID    Poll only interface <ID>
  -p,--position POS    Only poll poller item POS in the group
  -t,--type ID         Poll interfaces of type <ID>
...
function print_version()
{
  global $Config;
  print
"JFFNMS Version ".$Config->get('jffnms_version')."
";
die;
}
