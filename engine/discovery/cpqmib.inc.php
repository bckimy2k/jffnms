...
function discovery_cpqmib ($ip, $rocommunity, $hostid, $param)
{
  ...
  $fans   = '.1.3.6.1.4.1.232.6.2.6.7.1.';
  $temp   = '.1.3.6.1.4.1.232.6.2.6.8.1.';
  $power  = '.1.3.6.1.4.1.232.6.2.9.3.1.';
	...
	switch($disc_type) {
		...
		case 'powersupply':
			...
			if (is_array($chassis)) {
			    ...
			    $condition = snmp_walk($ip, $rocommunity, $power.'4');

			    foreach($chassis as $key => $cha) {
				$interfaces[$ifnum++] = array(
					'interface' => "Power $cha/$bayindex[$key]" ,
					'chassis' => $cha,
