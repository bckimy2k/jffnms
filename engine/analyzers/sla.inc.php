 ...
 */
	...
	$interface_data = $Interfaces->values($interface["id"],array("exclude_types"=>20));
	...
	while ($sla = db_fetch_array($result_sla)) {
	    ...
	    $sla_cond 			= replace_vars(trim($sla["cond"]),$values);
	...
	if (count($sla_cond_active)==1) {  // if there is only one result left
		...
		$Events->add(date("Y-m-d H:i:s",time()),$interface["event_type"],$interface["host"],$interface["interface"],
