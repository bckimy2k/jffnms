...
function consolidate_events(&$Events, &$Alarms, &$Triggers)
{
  $query_events = "
    SELECT  /*! HIGH_PRIORITY */ 
      events.id, events.date, events.type, alarm_states.state,
      interfaces.id as interface, alarm_states.id as alarm,
      types.alarm_up, types.alarm_duration
    FROM  events, types, interfaces, alarm_states
    WHERE
      events.interface != '' AND events.state != '' AND 
  ...
  {
    $processed = 0;
    if (($event['alarm_up']) && ($event['alarm_up'] > 1))
      $event['type'] = $event['alarm_up']; // override type
    logger("E $event[id]:= @$event[date] - state: $event[state] ".
      "- int: $event[interface] - type: $event[type]\n");

    //find a down
    $other_alarm = $Alarms->status($event['interface'],$event['type'],array(ALARM_DOWN,ALARM_TESTING)); //verify down or testing 
    logger("E $event[id]:= Other Down Alarms: $other_alarm[count]\n");
    if ($event['state']==ALARM_ALERT) //alert SLA or administrative
    {
      ...
      $Alarms->insert($Triggers, $event['date'],$date_stop,$event['interface'],$event['type'],$event['alarm'],$event['id'],$event['id']);
      $processed = 1;
      unset($date_stop);
    }
    if (($other_alarm['count'] > 0) && ($processed==0)) 
    {
      ...
      {
        $other_alarm['count']=0; //let the next if enter.
      ...
      $Alarms->update($other_alarm['alarm']['start_id'],$alarm_data); //bring up the older event
      //mark this, and the other alarm referer_start events as ACK (for events console)    
      $Events->set_ack($event['id'],1);
      $Events->set_ack($other_alarm['alarm']['referer_start'],1);
      $processed = 1;
      unset ($alarm_data);
    }

    if ((($event['state']==ALARM_DOWN) || ($event['state']==ALARM_TESTING)) &&
      ($other_alarm['count'] == 0))  //and we dont have any other active alarm, then is a DOWN/TESTING
    {
      logger("E $event[id]:= DOWN/TESTING Interface $event[interface]\n");
      $Alarms->insert($Triggers, $event['date'],'',$event['interface'],$event['type'],$event['alarm'],$event['id'],'');      
      $processed = 1;
    }

    if (($processed==1) || ($event['state']==ALARM_UP) ||
      ($event['state']==ALARM_DOWN) || ($event['state']==ALARM_TESTING))
    {
      $Events->set_analized($event['id']);
    }
  } // while
}
?>
