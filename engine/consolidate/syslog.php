...
function consolidate_syslog(&$Events)
{
  ...
  while ($syslog_record = db_fetch_array($result_syslog))
  {
		...
		$Events->add($syslog_record['date'],$event_type_id,$host_id,$values['interface'],strtolower($values['state']),$values['username'],$values['info'],$syslog_record['id']);
   db_update("syslog",$syslog_record['id'], array('analized'=>1));
	}	
}
?>
