...
function hosts_random_list($interval = false)
{
  $query_hosts="
...
function poller_status ($poller, $result)
{
  ...
  {
    logger (
      'Poller '.$poller['pid'].' finished working on host '.$poller['item'].
      ' in '.time_hms(round($time/1000)).", Items $items_ok/$items_total\n");
    return TRUE;
  }
  logger (
    'Poller '.$poller['pid'].' failed when polling host '.$poller["item"].
    ": $result\n");
  return FALSE;
}
...
{ 
  ...
  // Validate input
  if ($host_id != NULL && !is_numeric($host_id))
  {
    logger("ERROR: Host '$host_id' is not NULL or numeric.\n");
    return;
  }
  if ($interface_id != NULL && !is_numeric($interface_id) && $interface_id != 'dmii')
  {
    logger("ERROR: Interface '$interface_id' is not NULL, numeric or dmii.\n");
    return;
  }
  ...
  $time = round(time_msec_diff($time_start));
