...
function ad_hosts_list() { //FIXME add Satellite
  ...
  $params[2] = true;
    $params[3] = true;
    if (isset($_SERVER['argv'][3]) && $_SERVER['argv'][3]== 'nolog')
        $params[3] = false; // Do no insert events
...
function real_iad ($host_id = NULL, $type_id = NULL, $output = true, $log_events = true)
{
  ...
  $event_type_id = $Config->get('jffnms_administrative_type');
  ...
  $query_hosts='
    ...
    WHERE   hosts.id = '.$host_id.' 
     AND hosts.autodiscovery = autodiscovery.id
     AND hosts.autodiscovery > 1 and hosts.poll = 1';
  ...
  while ($host_info = db_fetch_array($result_hosts)) //this should return only one host
  {
    ...
    $query_ad='
      SELECT   id, autodiscovery_validate, autodiscovery_function,
      autodiscovery_parameters, autodiscovery_default_poller, 
      description, sla_default
    ...
    while ($it = db_fetch_array($result_ad)) //loop thru all interface_types
    {
      ...
      {
        ob_end_clean();
        // if $heartbeat seconds have passed since last time
        if (($old_time + $heartbeat) > time())
      ...
      discovery_logger($host_info['id'], $it['id'], FALSE, 
        'Autodiscovering '.$it['description']);

      //Get All nedded data for this interface type
      $db = hosts_interfaces_from_db ($host_info['id'],$it['id']);
  
      $host = hosts_interfaces_from_discovery ($it['autodiscovery_function'],
        $host_info['ip'], $host_info['rocommunity'], $host_info['id'],
        $it['autodiscovery_parameters']);

      $fields = interface_types_fields_list (NULL,
        array('itype'=>$it['id'],'exclude_types'=>20));
      ...
      if (is_array($host) && (count($host) > 0)) //if the host aswered something  
        foreach ($interface_ids_list as $key) if ($key >= 0) //key is valid 
        {
          ...
          //not found in DB and found in Host, add
          if (!isset($db[$key]) && isset($host[$key]))
          {
            if (($it['autodiscovery_validate']==0) //the interface type told us not to validate the data
              ...
              (array_key_exists('address', $host[$key]) && $host[$key]['address'] != '127.0.0.1') && //any loopback
            ...
            && ($it['autodiscovery_validate']==1) && ($db[$key]['poll'] > 1))  //IT need validation, and the interface is being polled (preserve non polling)
          {
            discovery_logger($host_info['id'], $it['id'], $key,
             ...
             if (($log_events==true) && ($host_info["alert_del"]==1))
               insert_event($now_date,$event_type_id, $host_id,
                 $db[$key]['interface'], 'alert','autodiscovery',
                 trim('Not Found in Host '.$text),0); 
             $processed = 1;
          } // delete
          //found in both and poll enabled, check for modification
          if (isset($host[$key]) && isset($db[$key]) && ($db[$key]['poll'] > 1))
          {
            //track field changes
            if ($it['autodiscovery_validate']==1) //validate fields in this interface type?
            {
              $fields_to_modify = array();
              $track_fields = array('interface' => 'Interface Name');
              foreach ($fields as $fdata)
                if ($fdata['tracked']==1) 
                  ...
                  && (trim(substr($db[$key][$track_field],0,30)) != trim(substr($host[$key][$track_field],0,30))) )  //fields not equal
                {
                  discovery_logger($host_info['id'], $it['id'], $key,
                    'RES : '.$track_field_descr.' Changed from '.
                      $db[$key][$track_field].' to '.
                      $host[$key][$track_field]);
              ...
              if (count($fields_to_modify) > 0)   //there are fields to be modified
              {
                ...
                if ($log_events==true)
                  insert_event($now_date, $event_type_id, $host_id,
                    $aux_interface_name, 'alert','autodiscovery',
                    trim('detected modification '.$aux_comment),0); 
                $processed = 1;
            ...
            if ($db[$key]['client'] <= 1) //customer not selected in DB
            {
              discovery_logger($host_info['id'], $it['id'], $key,
               ...
               if ($log_events==true)
                 insert_event($now_date, $event_type_id, $host_id,
                   $db[$key]['interface'], 'alert','autodiscovery',
                   'Incomplete Interface Setup (Customer not Selected)',0); 
               $processed = 1;
            }
          }// modification
