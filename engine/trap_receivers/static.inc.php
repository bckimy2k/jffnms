...
function trap_receiver_static ($params)
{
  ...
  $res = array();
  list ($res['status'], $interface_index_field, $interface_index_varbind)
    = explode(',', $params['receiver_parameters']);

  // Match interface (name and id) by index field
  if (($interface_index_field !== '') && ($interface_index_varbind !== ''))
  {
    $int_data = $Interfaces->get_by_field_value ($params['host_id'],
      $params['interface_type'], $interface_index_field, 
      $params['trap_vars'][$interface_index_varbind]);
